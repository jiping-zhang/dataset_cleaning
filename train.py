...
def train_model(tokenizer, model: torch.nn.Module, dataset: Dataset,
    ...
    if best_save_path is not None:
        if validset is None:
            raise ValueError('You have to provide a valid set if you wan\'t to save the best epoch during training')
    ...
    if tqdm_desc is None: tqdm_desc = str(gpu)
    for _ in tqdm(range(epoch), desc=tqdm_desc):
        ...
        for step, batch in enumerate(tqdm(data_loader, desc=tqdm_desc)):
            ...
            train_batch(model, input_dict, b_y, loss_func, optimizer,step%20==0)
        if best_save_path is not None:
...
def train_model_freelb(tokenizer, model: torch.nn.Module, dataset: Dataset,
    ...
    for _ in tqdm(range(epoch), desc=tqdm_desc):
        ...
        for step, batch in enumerate(tqdm(data_loader, desc=tqdm_desc)):
            ...
            input_dict = tokenizer(batch[0], return_tensors='pt', padding=True, truncation=True, max_length=max_len)
            to_device(input_dict, gpu)
            train_batch_freelb(model, input_dict, b_y, loss_func, optimizer, adv_lr=adv_lr, asteps=asteps)
            # output = model(**input_dict)
            # loss = loss_func(output.logits, b_y)
            # optimizer.zero_grad()
            # loss.backward()
